    ...
    try {
      ...
      if (device && device.group) {
        ...
        if (group && group.price) {
          ...
          const cost = hourlyRate * validDuration;

          console.log(`COST CALCULATION: ₹${hourlyRate} per hour × ${validDuration.toFixed(2)} hours = ₹${cost.toFixed(2)}`);

          // Set the session cost
          setSessionCost(cost);

          console.log(`Calculated session cost: ₹${cost.toFixed(2)} (${validDuration.toFixed(2)} hours at ₹${hourlyRate}/hour)`);
          return cost;
        } else {
  ...
  const createSession = useCallback(async () => {
    ...
    try {
      ...
      if (deviceId) {
        ...
        const cost = await fetchDeviceGroupAndCalculateCost(deviceId, 1); // 1 hour session

        // Update session data with calculated cost
        sessionData.session_total = cost || 0;
        sessionData.total_amount = cost || 0;
      }
    ...
    try {
      ...
      if (diff <= 0) {
        ...
        if (isSessionActive && sessionId) {
          console.log("Session time is up, updating status to Closed");
  ...
  const extendSession = useCallback(async () => {
    ...
    try {
      ...
      if (deviceInfo && deviceInfo.id) {
        try {
          // Calculate new cost based on extended duration
          newCost = await fetchDeviceGroupAndCalculateCost(deviceInfo.id, newDurationHours);
        } catch (costError) {
